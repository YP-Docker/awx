...
function ScheduleFormWizard({ isOpen, setIsOpen }) {
  ...
  const [{ value: frequenciesValue }, , frequenciesHelpers] =
    useField('frequencies');
  const [interval, , intervalHelpers] = useField('interval');

  const handleSubmit = (saveAndCreateNew, goToStepById) => {
    const {
      name,
      description,
      endingType,
    ...
    frequenciesHelpers.setValue(frequenciesValue);
    if (saveAndCreateNew) {
      goToStepById(1);
      return;
    }
